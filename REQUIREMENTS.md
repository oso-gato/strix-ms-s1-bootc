...
**Scope: clean workbench for host maintenance.** **GitHub authority (A19):** the box
operates GitHub under a **dedicated per-box GitHub App** — never a personal token,
never shared with another box. Credentials arrive via `firstboot.yaml`, persist with
the other secrets, and the host continuously mints short-lived installation tokens
the box consumes ambiently (`GH_TOKEN`); scope is set at the App level by the
operator. The rest of the fedora-bootstrap fleet-policy layer (managed-only
permission pinning, push-gate hooks, intervention loggers) stays **out of scope —
future**.
...
| A9 | 2026-07-11 | **DECLINED**: restic/borgbackup for the data drive. The operator accepts the recorded risk: SMART (surfaced by Cockpit → Storage) shows drive-health decline, but there is NO backup — drive loss = loss of homes, secrets, container volumes, and VM state. Preserve reinstalls protect against OS loss only. Revisit is a one-line ask. |
...
| A21 | 2026-07-13 | **On-hardware self-test tool** (R13 live-validation, no image change): `validation/strix-selftest.sh` in the public repo — a fresh box pulls and runs it on demand (`curl … | sudo bash`, read-only) to validate the running system against R1–R15 + objective + amendments, PASS/FAIL/WARN/SKIP with a non-zero exit on any FAIL. Actively exercises what CI/VM could not: LACP switch negotiation, amdgpu + gfx1151 in KFD topology, live `ttm.pages_limit`, a rootless container on the real GPU, NVMe SMART, and (`--deep`) ROCm ~120 GiB + VA-API. Not baked into the image (a runtime diagnostic, not a host capability) — no re-cut. |
| A20 | 2026-07-13 | **claudebox token renew command** (operator ergonomics for R8/A19). One-line root command `sudo strix-gh-renew` forces an immediate re-mint of the GitHub App installation token from the current persisted credentials (vs waiting for the ~50-min timer) and prints the new token's granted permissions + expiry + repository selection. Use after changing the App's permissions on GitHub (which must first be **approved** in the installation settings — a GitHub requirement, not scriptable) so the new scope goes live at once; also picks up a rotated private key if the persisted PEM was replaced (the minter re-reads creds each run). No new packages/units — thin wrapper over `strix-gh-app-token --report`. |
| A19 | 2026-07-13 | **R8 extension — claudebox GitHub authority (App-only)** (operator-approved; dual-identity fallback explicitly reverted). The claudebox operates GitHub under a **dedicated per-box GitHub App** identity — never a personal token, never shared with another box. Credentials (App ID + Installation ID + private-key PEM) arrive via an optional `github_app:` block in `firstboot.yaml`, persist to `/var/home/.strix-secrets/` with the other secrets (preserve-reinstall-restored), and the host continuously mints short-lived (~1 h) installation tokens the box consumes ambiently via `GH_TOKEN`. Scope is set at the App level by the operator. Consequence accepted: an App on a **personal** account cannot create new repos (create-repo needs a user token) — out of scope by choice; a new repo is created by the operator, then the App installed on it. No new host packages (JWT via base `openssl`). Verify teeth assert token freshness post-setup when the block was provided. |
| A18 | 2026-07-13 | **R15 Phase 2 — dynamic unified-memory ceiling** (operator picked **120 GiB**): bootc karg drop-in (`kargs.d/10-strix-uma.toml`, `ttm.pages_limit=31457280`) raises the GPU-mappable share of the 128 GB from the kernel's ~50% default to a 120 GiB **ceiling** — dynamic and reclaimable (never a reservation; idle cost zero); ~7.5 GiB can never be GPU-pinned, keeping host + a light VM alive at max load. BIOS dependency recorded (UMA frame buffer = 512 MB minimum, HARDWARE.md). Verify unit + CI assert the karg on the booted cmdline. **Deliberate exclusions**: `amd_iommu=off` (conflicts with IOMMU-on for NPU SVA + libvirt), `amdgpu.gttsize` (deprecated ≥6.17), `ttm.page_pool_size` (unmeasured perf knob — R1 minimalism). Grounded in the kyuz0 Strix-Halo community toolbox + AMD RDNA3.5 guidance + kernel source (all verified this session). |
...
| A13 | 2026-07-12 | **Tailscale SSH enabled** (`tailscale up --ssh`): keyless, tailnet-identity-authenticated shell over the tailnet, governed by the tailnet policy's `ssh` rules (operator adds e.g. `{"action":"accept","src":["autogroup:member"],"dst":["tag:router"],"users":["core"]}`). OpenSSH stays the LAN + mosh path with GitHub-synced keys; R4d/R5 hold (no password on any path; root unreachable — Tailscale SSH maps to `core` only per the ACL). Supersedes the noir-era exclusion: tailscale#12442 (SSH-vs-SELinux) closed 2024-08-06, and fedora-bootstrap runs `--ssh` in production on enforcing Fedora. Extends R3/R4. |
