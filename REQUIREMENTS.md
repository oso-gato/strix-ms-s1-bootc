...
| A15 | 2026-07-12 | **R10 `%pre` guard hardening** (hardware-invariant validation vs noir): added `udevadm settle \|\| sleep 3` at the top of the kickstart `%pre`, before the first `check_drive`. noir's serial/size/model guard ran under coreos-installer's already-settled live env; strix's `%pre` runs earlier under Anaconda, and on the two asymmetric-lane NVMes (x4 + x1) `/dev/disk/by-id` symlink timing isn't guaranteed — a not-yet-created symlink would trip `check_drive`'s existence test into a spurious fail-closed abort. VM-proven install path unaffected; this makes it deterministic on real hardware. *(Note: the cross-check of strix's bond0/Wi-Fi/drive config against noir's L3-proven settings found ZERO other gaps — byte-exact match; and the packaged `smartd.conf` DEVICESCAN already delivers A6 drive-health, so no smartd config was needed.)* |
